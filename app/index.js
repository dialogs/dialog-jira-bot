    ...
    flatMap(async message => {
      ...
      if (message.content.type === "text" &&
        ...
        let result = await axios({
          ...
          .catch(err => {
            console.log(err);
            bot.sendText(message.peer, "Unknown project code.", null)
          });
      } else if (message.content.type === "text" && command === USER_COMMAND) {
          getCurrentUser(bot, message.peer)
            .then(user => {
               console.log(user);
               axios({
                ...
                .then(response => {
                    ...
                    if (hasAssignee) bot.sendText(message.peer, inprogressIssues, null);
                })
                .catch(err => {
                    console.log(err);
                    bot.sendText(message.peer, "Oops. You don't seem to be on the list of users", null)
                })
            })
          ...
          fetchedProjects[message.peer.id].map(project => {
          ...
          let result = await axios({
            ...
            .then(response => {
              ...
              if (containsValue(tasksToTrack[message.peer.id], commandsArray[1])){
                 ...
                 console.log(tasksToTrack[message.peer.id]);
